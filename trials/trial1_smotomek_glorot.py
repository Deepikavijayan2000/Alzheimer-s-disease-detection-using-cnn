# -*- coding: utf-8 -*-
"""trial1 smotomek_glorot.ipynb

Automatically generated by Colab.

Original file is located at
    https://colab.research.google.com/drive/1JX15aPQypRG7Pfb_-M3BjctoE5VW9ziz
"""
...
train_dataset = datagen.flow_from_directory(WORKING_DIRECTORY, target_size=(176, 208),batch_size=3000, shuffle=True)
...
model.add(Conv2D(16, 3, kernel_initializer=init, activation='relu'))  # Smaller filter size (3x3) for local features
model.add(AveragePooling2D(pool_size=(2, 2)))

# Second convolutional block - Increased filter count, maintained filter size
model.add(Conv2D(32, 3, kernel_initializer=init, activation='relu'))
model.add(AveragePooling2D(pool_size=(2, 2)))

# Third convolutional block - Introduce max pooling for additional downsampling
model.add(Conv2D(64, 3, kernel_initializer=init, activation='relu'))
...
model.add(Conv2D(128, 5, kernel_initializer=init, activation='relu'))  # Larger filter size for capturing broader features
model.add(AveragePooling2D(pool_size=(2, 2)))
...
model.add(Convolution2D(16, 3, kernel_initializer=init))
model.add(ReLU())
model.add(AveragePooling2D(pool_size=(2,2)))
